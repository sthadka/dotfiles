...
parse_next2([T | Tokens], #state{stack = [T2 | _]} = State) when ?OPEN_BRACKET(T) ->
    case same_line(T, Tokens) of
        true ->
        ...
        false ->
            case T2 of
                {'=', _} ->
                    parse_next(Tokens, push(State, T, 1, column(T2) + 5));
                _ ->
                    parse_next(Tokens, push(State, T, 1))
            end
    end;
...
parse_next2([T | Tokens], State) when ?IS(T, 'fun'); ?IS(T, 'receive'); ?IS(T, 'if') ->
    parse_next(Tokens, indent_after(Tokens, push(State, T, 1, column(T) + 3), 2));
parse_next2([T | Tokens], State) when ?BRANCH_EXPR(T) ->
    parse_next(Tokens, push(State, T, 1, column(T) + 3));
...
parse_next2([T1 = {'->', _} | Tokens], State = #state{stack = [T2 | _]}) when ?BRANCH_EXPR(T2) ->
    parse_next(Tokens, push(unindent(State), T1, 1, column(T2) + 7));
